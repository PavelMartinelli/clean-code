...
public abstract class BoundaryTokenHandler : ITokenHandler
{
    ...
    public abstract bool CanHandle(char current, char next, ParserContext context);

    public void Handle(ParserContext context)
    {
        if (IsValidBoundary(context) && HasValidNesting(context))
    ...
    private void HandleTokenBoundary(ParserContext context)
    {
    ...
    private static int FindSingleDelimiter(string text, int startIndex, int paragraphEndIndex, string delimiter)
    {
        ...
        while (index != -1 && index < paragraphEndIndex)
        {
            ...
            if (index + delimiter.Length >= text.Length ||
                text[index + delimiter.Length] != '_') return index;
