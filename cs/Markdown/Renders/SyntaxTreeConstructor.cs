...
public class SyntaxTreeConstructor
{
    ...
    private void ProcessContainerToken(Token token, Stack<Token> stack,
        List<Token> result)
    {
    ...
    private void ProcessLeafToken(Token token, Stack<Token> stack,
        List<Token> result)
    {
        if (stack.Count > 0)
        {
    ...
    {
        return token.Type is TokenType.Strong or TokenType.Italics
            or TokenType.Header;
    }

    private bool CanNest(Token parent, Token child)
    {
        return nestingRules.TryGetValue(parent.Type,
                   out var allowedChildren) &&
    ...
    private void HandleTagIntersection(Stack<Token> stack, Token newToken,
        List<Token> result)
    {
